...
SAGE is a compact deterministic agent runtime. The central object is `AgentKernel`, which initializes state, builds a task graph, routes context retrieval, selects skills, dispatches specialist agents, records handoffs, calls typed tools, writes local artifacts, and verifies the run.
...
| Tool registry  | `tools.py`          | Typed tool use and structured errors       |
